...
def print_trainable_parameters(model):
    ...
    print(
        f"Trainable parameters: {trainable_params} || All parameters: {all_params} || Trainable % : {100 * trainable_params / all_params}"
    )
...
def define_train_args(config: DictConfig):
    training_args = transformers.TrainingArguments(
        ...
        eval_strategy=config.evaluation_strategy,  # Evaluate the model every logging step
...
class PrintLossCallback(TrainerCallback):
    ...
    def on_log(
        ...
        if "eval_loss" in state.log_history[-1]:
            logging.info(
                f"Step: {state.global_step}, Evaluation Loss: {state.log_history[-1]['eval_loss']}"
            )
...
def main_fn(cfg: DictConfig):
    ...
    data_vi = load_dataset(
        path=cfg_data.path_to_data,
        name="vi",
        cache_dir=cfg_data.cache_dir,
        split="train",
    )
    traindata, evaldata = select_small_sample(
        data_vi, cfg_data.nb_samples, cfg_data.split_ratio
    )

    logging.info("TRAIN DATA STRUCTURE : {}".format(traindata))

    tokenized_train = traindata.map(
        generate_and_tokenize_prompt, fn_kwargs={"tokenizer": tokenizer}
    )
    tokenized_eval = evaldata.map(
        generate_and_tokenize_prompt, fn_kwargs={"tokenizer": tokenizer}
    )
